...
describe('readStoredDashboardSession', () => {
  ...
  it('returns null and clears storage when sessionToken is not a string', () => {
    storeRaw(JSON.stringify({ sessionToken: { evil: true }, expiresAt: futureIso() }));
    expect(readStoredDashboardSession()).toBeNull();
    expect(window.localStorage.getItem(STORAGE_KEY)).toBeNull();
  });
...
describe('isDashboardAuthError', () => {
  ...
  it('returns true for ApiError 403 with Invalid Dashboard Token', () => {
    expect(isDashboardAuthError(new ApiError(403, 'Invalid Dashboard Token'))).toBe(true);
  });

  it('returns false for ApiError 403 used as a 2FA gate (not token rejection)', () => {
    expect(isDashboardAuthError(new ApiError(403, 'Invalid authenticator or backup code.'))).toBe(false);
  });
});

describe('scrubAuthQueryParamsFromUrl', () => {
  afterEach(() => {
    window.history.replaceState({}, '', '/');
  });

  it('removes token and session query params without leaving them in the URL', () => {
    window.history.replaceState({}, '', '/?token=secret-token&session=sess-1&keep=1');
    scrubAuthQueryParamsFromUrl();
  ...
  it('is a no-op when auth params are absent', () => {
