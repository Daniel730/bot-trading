...
class TestSlippageGuard(unittest.TestCase):
    ...
    @patch('src.services.data_service.data_service.get_latest_price')
    @patch('src.services.brokerage_service.BrokerageService.get_symbol_metadata')
    def test_place_value_order_buy_slippage(self, mock_metadata, mock_price, mock_post):
        ...
        self.service.place_value_order("AAPL", 100.0, "BUY")
    ...
    @patch('src.services.data_service.data_service.get_latest_price')
    @patch('src.services.brokerage_service.BrokerageService.get_symbol_metadata')
    def test_place_value_order_sell_slippage(self, mock_metadata, mock_price, mock_post):
        mock_price.return_value = {"AAPL": 100.0}
        mock_metadata.return_value = {"minTradeQuantity": 0.0, "quantityIncrement": 0.0}
        mock_post.return_value = MagicMock(status_code=200, json=lambda: {"id": "1"})

        # Sell $100 of AAPL at $100/share -> 1 share
        self.service.place_value_order("AAPL", 100.0, "SELL")
