...
**Feature Branch**: `004-strategic-arbitrage-engine`  
**Created**: 2026-03-28  
**Status**: Draft  
...
1. **Given** a statistical entry signal ($|Z| > 2.5$), **When** the system analyzes recent filings/news, **Then** it must provide a "Valid" (Technical Noise) or "Invalid" (Structural Change) recommendation.
2. **Given** an "Invalid" recommendation from the AI, **When** a signal is processed, **Then** the trade execution must be suppressed and the reasoning logged.
...
**Independent Test**: Can be tested by triggering a validated signal and verifying that the system calculates and executes the correct quantities for both assets to achieve the target rebalance.
...
1. **Given** Paper Trading mode is enabled, **When** a trade is "executed," **Then** no actual orders are sent to the brokerage, but the local virtual state is updated to reflect the simulated position.

### Edge Cases

- **Extreme Volatility**: Rapid price movements between signal generation and approval are handled by the slippage tolerance check (FR-011), which aborts the trade if the threshold is exceeded.
- **Zero Liquidity**: If one asset in a pair has temporarily halted trading or has zero liquidity, the system MUST abort the entire rebalance operation for that pair to maintain leg-dependency safety.
...
- **FR-004**: System MUST suppress execution if the AI validation identifies a "Structural Change" (fundamental event) or if the analysis is inconclusive due to insufficient data, notifying the user via Telegram for a manual decision.
