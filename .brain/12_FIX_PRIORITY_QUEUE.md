...
- DONE 2026-05-28: Alpaca ambiguous-submit timeout coverage is confirmed for both reconciled client-order-id recovery and unreconciled `unknown`/manual-reconciliation outcomes, with the release checklist guarded by `tests/unit/test_brain_ledgers.py`.
- DONE 2026-05-26: successful live monitor executions now resolve the original `LEG_A_SUBMITTED`/`LEG_B_SUBMITTED` ledger rows to final pair status instead of appending duplicate final rows and leaving stale startup-blocking submitted rows behind.
- DONE 2026-05-26: Leg B fill timeouts now fail closed to `NEEDS_MANUAL_RECONCILIATION`, preserve only confirmed Leg A fill data, and avoid recording requested Leg B quantity as a fill.
- DONE 2026-05-26: live close workflows now re-read broker quantities after confirmed close fills and block ledger closure with `NEEDS_MANUAL_RECONCILIATION` if any residual exposure remains or the verification read fails.
...
- DONE 2026-05-28: paper-mode wallet sync/no-broker-call coverage is confirmed for sync and recommendation-buy endpoints, with the release checklist guarded by `tests/unit/test_brain_ledgers.py`.
- DONE 2026-05-28: the focused execution-safety gate from `08_TESTING_PROTOCOL.md` was re-run against the split startup test files and passed with 59 tests, with the release checklist guarded by `tests/unit/test_brain_ledgers.py`.
- DONE 2026-05-28: the standalone dashboard wallet sync gate passed with 7 tests, with evidence recorded in `08_TESTING_PROTOCOL.md` and guarded by `tests/unit/test_brain_ledgers.py`.
...
- DONE 2026-05-26: Telegram and approval error diagnostics in `NotificationService` now use the module logger with token redaction instead of stdout prints, covered by `tests/unit/test_notification_service_paper.py`.
- DONE 2026-05-27: Paper approval notification scheduling now captures the notify callable before creating the background task, preventing tests from leaking into the real notify path after monkeypatch cleanup.
- DONE 2026-05-27: `test_request_approval_live_without_telegram_fails_closed` now mocks pause-state dependencies, preventing the fail-closed approval test from hanging on real service imports.
...
- DONE 2026-05-28: close-workflow tests now configure the shared `monitor` fixture instead of constructing a local `ArbitrageMonitor`, with a layout guard blocking local close-workflow monitor fixtures from returning.
